 ...
 */
var WASM_PATH =
  "https://unpkg.com/@rive-app/canvas-advanced-single@2.30.0/rive.wasm";
...
export class RiveSprite extends Sprite {
   ...
   */
  updateSize(): void {
    if (!this.artboard || !this._rive || !this._renderer) return;
    ...
    if (this.fit === Fit.Layout) {
      ...
      this._renderer.align(
        this._rive.Fit.contain, // any fit, we override with matrix
      ...
      this._aligned = this._rive.computeAlignment(
        fit,
        align,
        frame,
        this.artboard.bounds
